...
A Pi extension for **non-interactive** background shell jobs. It is intentionally not a PTY, terminal multiplexer, or interactive-shell replacement: jobs receive no stdin and run through `SHELL -c` (falling back to `/bin/sh`).
...
For development, run Pi with `pi -e ./src/index.ts` from this directory. The extension starts no processes, timers, or log directories until `background_job` is invoked.
...
stdout and stderr are captured together. Each job records start/end timestamps, exit code, signal, state, and bounded output metadata. Recent terminal jobs and logs are bounded (40 jobs, 256 KiB file capture, 16 KiB tail by default). Output is sanitised before display; no terminal escape sequences or hidden model reasoning are forwarded.
...
A global process-local singleton preserves running jobs and pending completions across `/reload`, then rebinds callbacks without duplicate notifications, timers, or listeners. Ordinary session shutdown stops active process groups, clears timers, and removes temporary log files. Child process groups are cleaned after their shell leader exits so ordinary `command &` descendants cannot silently escape; independently re-daemonized processes outside the original process group are outside this extension's ownership. Jobs do not survive a Pi process restart.
